...
for K8S_VERSION in $K8S_VERSIONS master; do
  ...
  if [ ! -d "${K8S_VERSION}-standalone-strict" ]; then
    $OPENAPI2JSONSCHEMABIN -o "schemas/master-standalone-strict" --expanded --kubernetes --stand-alone --strict "${SCHEMA}"
    $OPENAPI2JSONSCHEMABIN -o "schemas/master-standalone-strict" --kubernetes --stand-alone --strict "${SCHEMA}"
  fi

  if [ ! -d "${K8S_VERSION}-local" ]; then
    $OPENAPI2JSONSCHEMABIN -o "schemas/master-local/" --expanded --kubernetes "${SCHEMA}"
    $OPENAPI2JSONSCHEMABIN -o "schemas/master-local" --kubernetes "${SCHEMA}"
  fi
done
