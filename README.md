...
# The pre-built schema is already committed at custom-standalone-strict/agentconfig-v1beta1-v1beta1.json.
#
...
mv agentconfig-v1beta1-v1beta1.json custom-standalone-strict/
export FILENAME_FORMAT='{kind}-{fullgroup}-{version}'
...
| `custom-standalone-strict/agentconfig-v1beta1-v1beta1.json` | Generated JSON Schema consumed by kubeconform |
...
4. Commit both the updated `custom_crds/agent-config.yaml` and `custom-standalone-strict/agentconfig-v1beta1-v1beta1.json`.

> **Schema filename note:** `AgentConfig` uses `apiVersion: v1beta1` with no API
> group. kubeconform splits `apiVersion` on `/` to get `{{.Group}}` and
> `{{.ResourceAPIVersion}}`; with no `/`, both equal `v1beta1`. The
> `{{.ResourceKind}}-{{.Group}}-{{.ResourceAPIVersion}}` schema-location template
> (matching the `FILENAME_FORMAT='{kind}-{fullgroup}-{version}'` convention used
> by `openapi2jsonschema.py`) therefore resolves to `agentconfig-v1beta1-v1beta1.json`.
> The CRD YAML uses `spec.group: "v1beta1"` to make `openapi2jsonschema.py`
