...
> `{kind}-{group}-{version}`. For `apiVersion: v1beta1` (no `/`), kubeconform
> treats the whole string as both `Group` and `ResourceAPIVersion`, so both
> equal `v1beta1` and the filename is `agentconfig-v1beta1-v1beta1.json`.
